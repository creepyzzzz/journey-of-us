...
import type { MultiplayerGameRoom, PlayerInfo, TurnInfo, WheelSpin } from './types';
...
interface MultiplayerStore {
  ...
  // Actions
  createRoom: (gameId: string, hostName: string) => Promise<MultiplayerGameRoom>;
  joinRoom: (gameCode: string, playerName: string) => Promise<MultiplayerGameRoom>;
...
export const useMultiplayerStore = create<MultiplayerStore>((set, get) => ({
  ...
  leaveRoom: async () => {
    ...
    const updatedPlayers = currentRoom.joinedPlayers.filter(p => p.id !== playerId);
  ...
  spinWheel: async (levelId: number, questionId?: string) => {
    ...
    const results: ('player1' | 'player2' | 'both')[] = ['player1', 'player2', 'both'];
    ...
    const currentTurn: TurnInfo = {
      playerId: randomResult === 'both' ? 'both' : currentRoom.joinedPlayers.find(p => 
        randomResult === 'player1' ? p.id === currentRoom.joinedPlayers[0].id : p.id === currentRoom.joinedPlayers[1].id
      )?.id || '',
      playerName: randomResult === 'both' ? 'Both Players' : currentRoom.joinedPlayers.find(p => 
        randomResult === 'player1' ? p.id === currentRoom.joinedPlayers[0].id : p.id === currentRoom.joinedPlayers[1].id
      )?.name || '',
